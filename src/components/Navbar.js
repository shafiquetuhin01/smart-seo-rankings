...
const Navbar = () => {
  const Links = [
    {
      icon1: <ion-icon style={{fontSize:'45px',fontWeight:'bold'}}  name="home-outline"></ion-icon>,
      link: "/home",
    },
    {
      name: "SEO services",
        ...
        >
          {Links.map((link) => (
            <li
              ...
              className="w-48 h-20 text-right font-semibold text-white text-base py-2 hover:bg-primary capitalize"
            >
              <a href={link.link} className="flex items-center justify-center break-words align-middle">
                <span className="text-3xl my-3">{link.icon1}</span>
                <span>{link.name}</span>
